...
Most optimizations from this doc have been implemented. See "Still Open" for remaining ideas and "Completed" for what's done.
...
- **`flee_to_corridor`** — Pathfind to nearest 1-wide corridor for tactical positioning. Could be built on existing pathfinding.
...
- **`auto_explore`** — Finds nearest frontier, pathfinds to it. Stops for monsters, damage, or when frontier is reached.
- **`pathfind_to(x, y)`** — A\* pathfind to any visible/explored coordinate. Stops for monsters or damage.
- **`auto_fight`** — Resolves combat with the weakest adjacent monster to the death in one call. Returns fight metadata (rounds, HP lost, target killed).

### Smarter Autorun

- Autorun tuned to stop only at meaningful decision points (corridor forks, room exits), not every open-neighbor junction.
...
**Layer 2 — Exploration graph (always sent when changed, structured JSON)** (`crates/core/src/exploration_graph.rs`, integrated into MCP server)
...
**Layer 3 — Delta/conditional (only when changed)** (fingerprint-based delta compression in `mcp_server.rs`)
...
**Layer 4 — Full ASCII map (on-demand via separate tool)** (`get_explored_map` MCP tool)
...
| Compact mode | Omits ASCII map, shortens field names |
