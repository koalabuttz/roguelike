...
/// Sprite frame data: 5 source frames × 64 bytes (63 pixel data + 1 padding).
/// Designed in Spritemate, singlecolor, color $03 (cyan).
/// Frames 0,1,2,5,6 stored; frames 3,4,7 derived at runtime via vertical flip:
///   Frame 3 (SE) = vflip(Frame 1 NE)
///   Frame 4 (S)  = vflip(Frame 0 N)
///   Frame 7 (NW) = vflip(Frame 5 SW)
/// Stored in logical order (0,1,2,5,6) — copied to non-contiguous RAM slots.
const SPRITE_DATA: [u8; 320] = [
    // [0] Frame 0: vertical sword pointing up (N) → RAM $0340
    ...
    // [1] Frame 1: NE diagonal → RAM $0380
    ...
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    // [2] Frame 2: horizontal sword pointing right (E) → RAM $03C0
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    // [3] Frame 5: SW diagonal → RAM $0480
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    // [4] Frame 6: horizontal sword pointing left (W) → RAM $04C0
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
/// Generate a vertically-flipped sprite frame: reverse row order.
...
pub fn spinner_start() {
    unsafe {
        ...
        // Copy 5 source frames to their RAM slots (two contiguous runs)
        // SPRITE_DATA[0..192]  = frames 0,1,2 → slots 0,1,2 ($0340-$043F)
        for i in 0..192u16 {
            write_volatile(dst.add(i as usize), *src.add(i as usize));
        }
        // SPRITE_DATA[192..320] = frames 5,6 → slots 5,6 ($0480-$04FF)
        for i in 0..128u16 {
            write_volatile(dst.add(5 * 64 + i as usize), *src.add(192 + i as usize));
        }

        // Derive 3 frames via vertical flip (reverse row order)
        let frame = |idx: u16| dst.add(idx as usize * 64);
        sprite_vflip(frame(1), frame(3));   // Frame 3 (SE) = vflip(Frame 1 NE)
        sprite_vflip(frame(0), frame(4));   // Frame 4 (S)  = vflip(Frame 0 N)
        sprite_vflip(frame(5), frame(7));   // Frame 7 (NW) = vflip(Frame 5 SW)
