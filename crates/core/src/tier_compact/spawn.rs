...
pub fn spawn_monsters(entities: &mut EntityStore, map: &CompactMap, rng: &mut LfsrRng32) {
    for ri in 1..map.room_count {
        ...
        for _ in 0..count {
            ...
            let mx = rng.range_u8(room.x as u8 + 1, (room.x + room.w - 1) as u8) as Coord;
            let my = rng.range_u8(room.y as u8 + 1, (room.y + room.h - 1) as u8) as Coord;
...
pub fn spawn_items(items: &mut ItemStore, map: &CompactMap, depth: u8, rng: &mut LfsrRng32) {
    for ri in 1..map.room_count {
        let room = map.rooms[ri as usize];
        ...
        for _ in 0..count {
            ...
            let ix = rng.range_u8(room.x as u8 + 1, (room.x + room.w - 1) as u8) as Coord;
            let iy = rng.range_u8(room.y as u8 + 1, (room.y + room.h - 1) as u8) as Coord;
