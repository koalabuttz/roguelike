...
use crate::tier_compact::autorun::{
    ...
    stairs_in_fov as compact_stairs_in_fov,
};
use crate::tier_compact::game::CompactGameState;
use crate::tier_compact::pathfinding as compact_pathfinding;
...
pub fn create_game(
    ...
    match seed_code::tier_from_seed(seed) {
        ...
        seed_code::Tier::Compact => {
            use crate::tier_compact::types::{MAP_HEIGHT, MAP_SIZE, MAP_WIDTH};
            let w = width;
            let h = height;
            if w < balance::MIN_MAP_WIDTH as i32 || h < balance::MIN_MAP_HEIGHT as i32 {
                return Err(format!(
                    "Map must be at least {}x{} tiles",
                    balance::MIN_MAP_WIDTH,
                    balance::MIN_MAP_HEIGHT
            ...
            if (w as usize) * (h as usize) > MAP_SIZE {
                return Err(format!(
                    "Compact-tier map must fit within {}x{} tiles ({})",
                    MAP_WIDTH, MAP_HEIGHT, MAP_SIZE
                ));
            }
            Ok(Box::new(CompactGameStateAdapter::new(seed as u32, w, h)))
        }
...
// ── Compact tier adapter ────────────────────────────────────────────

/// Adapter wrapping `CompactGameState` to implement `GameStep`.
///
/// The compact tier uses `i32` coordinates natively, so no coord widening
/// is needed (unlike the micro adapter). Stats are widened from `u8`.
pub struct CompactGameStateAdapter {
    pub game: CompactGameState,
    seed: u32,
}

impl CompactGameStateAdapter {
    pub fn new(seed: u32, width: i32, height: i32) -> Self {
        Self {
            game: CompactGameState::new(seed, width, height),
            seed,
        }
    }
    ...
    pub fn autorun(&mut self, dir: Direction) -> AutorunResult {
        let explored_before = self.game.fov.explored_floor_count(&self.game.map) as Stat;
        let mut stepper =
            CompactAutorunStepper::new(dir, compact_stairs_in_fov(&self.game.map, &self.game.fov));
        let mut all_messages = Vec::new();
        loop {
            let msg_before = self.game.log.total();
            let outcome = stepper.next_step(&mut self.game);
            collect_compact_messages(&self.game.log, msg_before, &mut all_messages);
            match outcome {
                CompactStepOutcome::Continue => continue,
                CompactStepOutcome::Done(stop) => {
                    let explored_after = self.game.fov.explored_floor_count(&self.game.map) as Stat;
                    return AutorunResult {
                        steps_taken: stepper.steps_taken() as Stat,
                        stop_reason: map_compact_stop_reason(stop),
                        messages: all_messages,
                        new_tiles_revealed: explored_after - explored_before,
                    };
                }
    ...
    pub fn pathfind_to(&mut self, tx: i32, ty: i32) -> Result<AutorunResult, String> {
        if !self.game.map.in_bounds(tx, ty) {
            return Err("Target is out of bounds".into());
        }
        if !self.game.fov.is_explored(tx, ty) {
            return Err("Target tile has not been explored".into());
        ...
        let explored_before = self.game.fov.explored_floor_count(&self.game.map) as Stat;
        let mut buf = compact_pathfinding::BfsBuffers::new();
        let mut stepper = CompactBfsStepper::new(
            tx,
            ty,
            compact_stairs_in_fov(&self.game.map, &self.game.fov),
        );
        let mut all_messages = Vec::new();
    ...
    pub fn auto_explore(&mut self) -> Result<AutoExploreResult, String> {
        ...
        let mut buf = compact_pathfinding::BfsBuffers::new();
        let (tx, ty) = compact_pathfinding::find_nearest_frontier(
            px,
            py,
            &self.game.map,
            &self.game.fov,
        ...
        let movement = self.pathfind_to(tx, ty)?;
        Ok(AutoExploreResult {
            target_x: tx as Coord,
            target_y: ty as Coord,
            movement,
    ...
    pub fn frontier_count(&self) -> i32 {
        let pi = crate::tier_compact::types::PLAYER_IDX as usize;
        let mut buf = compact_pathfinding::BfsBuffers::new();
        compact_pathfinding::frontier_count(
            self.game.entities.x[pi],
            self.game.entities.y[pi],
            &self.game.map,
            &self.game.fov,
    ...
    pub fn auto_fight(&mut self) -> Result<AutoFightResult, String> {
        ...
        let mut messages = Vec::new();
        collect_compact_messages(&self.game.log, msg_before, &mut messages);
...
impl GameStep for CompactGameStateAdapter {
    fn step(&mut self, cmd: GameCommand) -> StepResult {
        ...
        collect_compact_messages(&self.game.log, msg_before, &mut new_messages);
    ...
    fn observe(&self) -> GameObservation {
        ...
        let map_ascii = build_compact_map_ascii(map, fov, entities, &self.game.items);
        let visible_entities = build_compact_visible_entities(entities, fov);
        let visible_items = build_compact_visible_items(&self.game.items, fov);
        let recent_messages = build_compact_recent_messages(&self.game.log);
        ...
        GameObservation {
            ...
            player_atk: self.game.effective_attack() as i32,
            player_def: self.game.effective_defense() as i32,
            player_x: entities.x[pi],
            player_y: entities.y[pi],
            map_ascii,
            visible_entities,
            visible_items,
            recent_messages,
            ...
            target_depth: balance::TARGET_DEPTH as i32,
            game_won: self.game.game_won,
            stairs: find_compact_explored_stairs(map, fov),
        }
    }
    ...
    fn look_at(&self, x: i32, y: i32) -> TileInfo {
        ...
        let entity = if visible {
            compact_entity_info_at(&self.game.entities, x, y)
        } else {
            None
        };

        let items_at: Vec<ItemInfo> = if visible {
            build_compact_items_at(&self.game.items, x, y)
        } else {
            Vec::new()
        };
...
fn map_compact_stop_reason(stop: CompactAutorunStop) -> AutorunStopReason {
    match stop {
        CompactAutorunStop::WallReached => AutorunStopReason::WallReached,
        CompactAutorunStop::MonsterSpotted => AutorunStopReason::MonsterSpotted,
        CompactAutorunStop::DamageTaken => AutorunStopReason::DamageTaken,
        CompactAutorunStop::GameOver => AutorunStopReason::GameOver,
        CompactAutorunStop::CorridorBranches => AutorunStopReason::CorridorBranches,
        CompactAutorunStop::MaxSteps => AutorunStopReason::MaxSteps,
        CompactAutorunStop::PathComplete => AutorunStopReason::PathComplete,
        CompactAutorunStop::StairsFound => AutorunStopReason::StairsFound,
    }
}

fn collect_compact_messages(
    log: &crate::tier_compact::msglog::CompactMessageLog,
    msg_count_before: u16,
    out: &mut Vec<String>,
) {
    const MSG_COUNT: u16 = 8;
    let new_count = log.total();
    let added = new_count.wrapping_sub(msg_count_before).min(MSG_COUNT) as u8;
    for i in 0..added {
        if let Some(evt) = log.recent(added - 1 - i) {
            out.push(format_event(evt));
        }
    }
}

fn build_compact_map_ascii(
    map: &crate::tier_compact::map::CompactMap,
    fov: &crate::tier_compact::fov::CompactFov,
    entities: &crate::tier_compact::entity::EntityStore,
    items: &crate::tier_compact::item_store::ItemStore,
) -> Vec<String> {
    let mut lines = Vec::new();
    for y in 0..map.height {
        let mut line = String::with_capacity(map.width as usize);
        ...
        for x in 0..map.width {
            if fov.is_visible(x, y) {
                has_content = true;
                if let Some(ei) = compact_entity_info_at(entities, x, y) {
                    line.push(ei.glyph);
                } else if let Some(glyph) = compact_item_glyph_at(items, x, y) {
                    line.push(glyph);
                } else {
                    line.push(tile_glyph(map.tile_at(x, y)));
                }
...
fn build_compact_visible_entities(
    entities: &crate::tier_compact::entity::EntityStore,
    fov: &crate::tier_compact::fov::CompactFov,
) -> Vec<EntityInfo> {
    let mut result = Vec::new();
    for i in 1..entities.count as usize {
        if entities.alive[i] && fov.is_visible(entities.x[i], entities.y[i]) {
            let (name, glyph) = if let Some(k) = entities.kind[i] {
                (monster_table::name(k).to_string(), monster_table::glyph(k))
            } else {
                ("Unknown".to_string(), '?')
            };
...
fn build_compact_visible_items(
    items: &crate::tier_compact::item_store::ItemStore,
    fov: &crate::tier_compact::fov::CompactFov,
) -> Vec<ItemInfo> {
    let mut result = Vec::new();
...
fn build_compact_recent_messages(
    log: &crate::tier_compact::msglog::CompactMessageLog,
) -> Vec<String> {
    let mut messages = Vec::new();
    for i in (0..8u8).rev() {
...
fn find_compact_explored_stairs(
    map: &crate::tier_compact::map::CompactMap,
    fov: &crate::tier_compact::fov::CompactFov,
) -> Option<(Coord, Coord)> {
    for y in 0..map.height {
        for x in 0..map.width {
            if fov.is_explored(x, y)
                && map.tile_at(x, y) == crate::tier_compact::map::TILE_STAIRS_DOWN
            {
                return Some((x, y));
            }
        }
