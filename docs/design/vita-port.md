...
Unlike the GBA port, which needs a `no_std` refactor, `heapless` containers, and `i16`/`i8` type sizing, the Vita:

- Has a full operating system with `std` support via vitasdk's newlib
- Uses the same `i32` types as desktop (`Coord`, `Stat`, `Pos`)
...
| D-pad diagonals | `Move { dx, dy }` | Hardware supports simultaneous Up+Right etc. — true 8-direction unlike GBA |
| Left analog stick | `Move { dx, dy }` | 8-direction with dead zone. Octagonal sector mapping. |
...
```rust
fn analog_to_direction(x: i8, y: i8) -> Option<(Coord, Coord)> {
    ...
    Some(DIRECTION_TABLE[sector])  // [(1,0), (1,1), (0,1), (-1,1), ...]
}
```
...
The [C64 port proposal](../c64-port-proposal.md#312-networking-uii-features) defines server-side endpoints for leaderboards, daily seeds, cloud saves, and spectation relays — designed to serve both C64 (via UII+ Ethernet) and other clients. The Vita consumes these same endpoints over WiFi, with the advantage of native HTTPS support (no bridge needed, unlike the C64's binary TCP protocol through the UII+).
...
- **Test:** Seed codes are shareable. Leaderboard submissions work. Network features degrade gracefully when WiFi is off (same principle as the C64's [graceful UII+ absence detection](../c64-port-proposal.md#312-networking-uii-features)).
...
6. **Cross-platform leaderboard fairness.** The [C64 proposal raises this question](../c64-port-proposal.md#11-open-questions) (Q8): C64 uses 40x22 maps, desktop/Vita use 80x40. Should daily challenge leaderboards be per-platform or require a shared map size? The Vita can generate maps at any size, so it could use C64-sized maps for daily challenges and full-sized maps for freeplay. This is a design decision that affects all networked ports.

7. **Cross-platform save portability.** The C64's binary save format (~1.6 KB) and the Vita's JSON format (~10-50 KB) represent the same game state in incompatible serializations. The [C64 proposal](../c64-port-proposal.md#11-open-questions) (Q12) and the [C64 bridge doc](c64-atproto-bridge.md) discuss this at length. If atproto PDS saves are schema-compatible at the lexicon level, a player could in theory continue a C64 game on a Vita (or vice versa) — but this requires a conversion layer. Is this a goal for v1, or aspirational?
...
|---------|----------|-----|-----|-----|----------|-----|-----|
| `std` available | Yes | Yes | Yes | No | **Yes** | N/A (6502 asm) | Yes |
| `SaveBackend` | Yes | Yes | N/A | No (SRAM) | **Yes** | No (binary) | Yes |
| `no_std` needed | No | No | No | Yes | **No** | N/A (not Rust) | No |
| Type sizing | i32 | i32 | i32 | i16/i8 | **i32** | u8/i8 (native) | i32 |
...
The Vita port sits in a sweet spot: it has the hardware richness to be the most feature-complete port, while requiring zero core modifications. It doesn't block or depend on any other port's work. The GBA's `no_std` refactor, the C64's ground-up 6502 reimplementation, the Web's async constraints — none of these affect the Vita.
...
The C64 port is architecturally unique among all ports: it's a **ground-up reimplementation in 6502 assembly**, not a Rust workspace crate. It reimplements core game logic from scratch, adapting algorithms for 8-bit constraints (Bresenham raycasting instead of shadowcasting, 16-bit LFSR instead of `rand`, fixed entity tables instead of `Vec`). The Vita port is the opposite extreme — it reuses `roguelike-core` entirely unchanged, with all effort going into the frontend.

This distinction matters for save compatibility. The C64's binary save format (~1.6 KB, see [C64 doc section 3.10](../c64-port-proposal.md#310-save-system)) is structurally different from the JSON `GameState` serialization used by the Vita and all other Rust-based ports. Cross-platform save portability between C64 and Vita/desktop requires format conversion — either in the atproto bridge (as the [C64 bridge doc](c64-atproto-bridge.md) proposes) or as a server-side translation layer. The Vita doesn't need to solve this problem directly, but its atproto integration should use the same lexicons (`save.gameState`, `save.settings`) so that saves are at least schema-compatible at the PDS level.
...
The Vita port is significantly less effort than the GBA port (no `no_std` refactor, no container changes, no type sizing) while delivering a richer end product. This is the most favorable effort-to-impressiveness ratio of any planned port.
