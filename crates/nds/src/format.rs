...
//!
//! Port of `crates/gba/src/format.rs` adapted for the 32-column DS screen.
...
/// Format a GameEvent into a fixed-size ASCII buffer (32 bytes for DS screen width).
/// Returns the number of bytes written.
pub fn format_event(event: GameEvent, buf: &mut [u8; 32]) -> usize {
    buf.fill(b' ');
    format_event_inner(event, buf).min(32)
}

fn format_event_inner(event: GameEvent, buf: &mut [u8; 32]) -> usize {
    match event {
